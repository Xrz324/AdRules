...
STRICT_UPSTREAM_DOWNLOAD="${STRICT_UPSTREAM_DOWNLOAD:-false}"

# 检查依赖
check_deps basename curl cut md5sum sed xargs
...
if [ -s "$FAILED_LOG" ]; then
    log_error "以下链接下载失败:"
    cat "$FAILED_LOG"
    if [[ "$STRICT_UPSTREAM_DOWNLOAD" == "true" ]]; then
        log_error "STRICT_UPSTREAM_DOWNLOAD=true，终止流程"
        exit 1
    fi
fi
